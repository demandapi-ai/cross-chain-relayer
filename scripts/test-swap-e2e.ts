...
async function main() {
    ...
    try {
        ...
        console.log(`Tx Success: ${(response as any).success}`);
        console.log(`VM Status: ${(response as any).vm_status}`);

        // Try to find the event
        const events = (response as any).events || [];
        console.log(`\nEvents in transaction: ${events.length}`);

        let foundEscrowId: number | null = null;
        ...
        try {
            ...
            console.log(`Claim Success: ${(claimResponse as any).success}`);
            console.log(`VM Status: ${(claimResponse as any).vm_status}`);
