...
async function main() {
    ...
    try {
        ...
        const response: any = await aptos.waitForTransaction({ transactionHash: pendingTx.hash });
        console.log(`Test H Success: ${response.success}, VM Status: ${response.vm_status}`);
    } catch (e: any) {
        console.error("FAILED Test H:", e.message || e);
        if (JSON.stringify(e).includes("0xa")) console.log("-> 0xa (INVALID)");
        if (JSON.stringify(e).includes("0x8")) console.log("-> 0x8 (EXPIRED)");
    }
}
