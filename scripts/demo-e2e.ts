...
// Colors
const LOG_INFO = chalk.blue;
const LOG_SUCCESS = chalk.green;
const LOG_WARN = chalk.yellow;
const LOG_ERROR = chalk.red;
const LOG_DATA = chalk.gray;
...
async function main() {
    console.clear();
    console.log(chalk.bold.magenta('🚀 Cross-Chain Relayer E2E Demo 🚀\n'));
    ...
    // Movement User
    const movUser = Account.generate();
    ...
    try {
        ...
        const sig = await solConnection.requestAirdrop(solUser.publicKey, 1 * LAMPORTS_PER_SOL);
        ...
        try {
            ...
            if (solKeyRaw) {
                ...
                const funder = Keypair.fromSecretKey(solKey);
                ...
                const sig = await sendAndConfirmTransaction(solConnection, tx, [funder]);
    ...
    if (relayerKey) {
        try {
            ...
            const privateKey = new Ed25519PrivateKey(relayerKey);
            const funder = Account.fromPrivateKey({ privateKey });
    ...
    console.log(chalk.bold(`   💰 User Balances: ${solBal / LAMPORTS_PER_SOL} SOL | ${movBal / 1e8} MOVE`));
    ...
    try {
        ...
        targetEscrowId = parseInt(stats[0] as string);
    ...
    const RELAYER_SOL_ADDR = new PublicKey("Crgx1n8HxZe8fq3d3vYhYrLDuKJ3pJZN4gfeSKuNarub"); // Hardcoded from known key or config
    ...
    try {
        ...
        const revealRes = await axios.post(`${RELAYER_URL}/reveal-secret`, {
            intentId,
            secret
        });
        ...
        if (e.response) console.log(LOG_DATA(JSON.stringify(e.response.data)));
    }
    ...
    const secretBytes2 = randomBytes(32);
    const hashlockBytes2 = createHash('sha256').update(secretBytes2).digest();
    ...
    try {
        ...
        if (events) {
            const createEvent = events.find((e: any) => e.type.includes("EscrowCreatedEvent"));
            if (createEvent) {
        ...
        // 6c. Notify Relayer
        console.log(LOG_DATA('   Step B: Notifying Relayer...'));
        const intentRes2 = await axios.post(`${RELAYER_URL}/swap/movement-to-solana`, {
    ...
    // Relayer Finals
    const healthFinal = await axios.get(`${RELAYER_URL}/health`);
    ...
    const userSolDelta = (solBalFinal - solBal) / LAMPORTS_PER_SOL;
    ...
    const fmt = (n: number, sym: string) => {
        const s = n > 0 ? '+' : '';
        const color = n > 0 ? chalk.green : (n < 0 ? chalk.red : chalk.gray);
